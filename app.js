...
function parseUploadedWorkbook(wb) {
  ...
  /**
   * 기존 정보(state.info)와 새로 읽은 newInfo를 병합하는 대신, 엑셀에 있는 값을 기준으로 해당 섹션을 통째로 교체합니다.
   *
   * 이전 구현에서는 기존 라벨을 유지한 채 같은 라벨이 있으면 업데이트하고 없으면 추가했습니다.
   * 이 방식은 엑셀에서 특정 항목을 제거했을 때 기존 데이터가 그대로 남아있는 문제가 있었습니다.
   *
   * 여기서는 newInfo에 존재하는 라벨들만 사용하여 각 섹션을 새로 생성합니다.
   * 만약 newInfo의 특정 섹션 배열이 비어있다면 parseUploadedWorkbook 상단에서 기본값을 채우므로
   * 여기서는 단순히 해당 배열을 깊은 복제하여 사용하면 됩니다.
   */
  const mergedInfo = {};
  for (const k of ["school", "bank", "card"]) {
    const rowsToMerge = Array.isArray(newInfo[k]) ? newInfo[k] : [];
    // 깊은 복제: 배열의 각 요소를 새 배열로 매핑하여 참조 분리
    mergedInfo[k] = rowsToMerge.map(([label, value]) => [label, value]);
  }
